<template>
  <div class="page">
    <van-nav-bar
      title="支付结果"
      left-text="返回"
      left-arrow
      @click-left="goRecords"
    />

    <div class="return-body">
      <van-loading v-if="checking" size="48" />
      <template v-else>
        <van-icon name="checked" size="64" color="#07c160" />
        <h2>支付完成</h2>
        <p class="payment-no" v-if="tradeNo">交易号：{{ tradeNo }}</p>
        <p class="tip">{{ countdown }} 秒后自动跳转至支付记录</p>
        <div class="actions">
          <van-button type="primary" block round @click="goRecords"
            >查看支付记录</van-button
          >
          <van-button plain block round style="margin-top: 12px" @click="goHome"
            >返回首页</van-button
          >
        </div>
      </template>
    </div>
  </div>
</template>

<script setup lang="ts">
import { ref, onMounted, onUnmounted } from "vue";
import { useRoute, useRouter } from "vue-router";

const route = useRoute();
const router = useRouter();

const checking = ref(true);
const tradeNo = ref("");
const countdown = ref(3);
let timer: ReturnType<typeof setInterval> | null = null;

function goHome() {
  router.push("/");
}
function goRecords() {
  router.push("/records");
}

onMounted(() => {
  // 清除支付标记（正常回跳已到达，无需 pageshow 兜底）
  sessionStorage.removeItem("__paying__");

  // 提取支付宝返回参数
  tradeNo.value = (route.query.trade_no as string) || "";

  // 短暂展示加载状态（模拟回跳处理）
  setTimeout(() => {
    checking.value = false;
  }, 600);

  // 3 秒倒计时自动跳转
  timer = setInterval(() => {
    countdown.value--;
    if (countdown.value <= 0) {
      clearInterval(timer!);
      router.replace("/records");
    }
  }, 1000);
});

onUnmounted(() => {
  if (timer) clearInterval(timer);
});
</script>

<style scoped>
.page {
  min-height: 100vh;
  background: #fff;
}

.return-body {
  display: flex;
  flex-direction: column;
  align-items: center;
  padding: 80px 24px;
  text-align: center;
}

.return-body h2 {
  margin: 16px 0 8px;
  font-size: 20px;
}

.payment-no {
  font-size: 13px;
  color: #909399;
  word-break: break-all;
}

.tip {
  font-size: 13px;
  color: #c0c4cc;
  margin: 8px 0 32px;
}

.actions {
  width: 100%;
  max-width: 300px;
}
</style>
